" $Date: Sat Jul 06 20:31:23 2013 +0800
...
" ---------------------------------------------------------------------
" Replace Chinese Symbol:			" have unexpected problems f[[
imap （ (
imap ） )
imap 』 }
imap 『 {
...
" There are useless since clang are integrated in YCM
...
" Use * to Search word under cusor
...
let g:fuf_coveragefile_prompt       = '>ProjectFile[]>'
...
nmap <Leader>js :call JsBeautify()<CR>:set ft=json<CR>
...
" ,rf to start; ,ff to send function; ,l to send line; ,se to send selection;
" ,rm to clear all; ,rr to clear console; ,ro to view object; ,ra to view arguments
" :RUpdateObjList to refresh
" <C-X><C-A> to complete arguments
let vimrplugin_notmuxconf = 1
let vimrplugin_term = g:my_term
let vimrplugin_listmethods = 1
let vimrplugin_ca_ck = 1			" delete partial shell content
...
func Tex_Block(label)
	...
	if has_key(Blk_Dict, a:label)
		let blk_text = Blk_Dict[a:label ]
	else
		let blk_text = a:label
	endif
